...
public class BoardTest {
    ...
    private ReadOnlyIntegerWrapper blackScore;
    private ReadOnlyIntegerWrapper whiteScore;
    private ReadOnlyIntegerWrapper move; // Current move number
    ...
    public BoardTest() {
        ...
        activePlayer = 0;
        blackScore = new ReadOnlyIntegerWrapper(2);
        whiteScore = new ReadOnlyIntegerWrapper(2);
        move = new ReadOnlyIntegerWrapper(0);
        gameOver = false;
    ...
    public boolean move(int player, int pos) {
        ...
        if (disksToFlip == 0) { // If this is not a valid move
            ...
            if (player == 0) {
                blackScore.set(blackScore.get() + disksFlipped + 1);
                whiteScore.set(whiteScore.get() - disksFlipped);
            } else {
                whiteScore.set(whiteScore.get() + disksFlipped + 1);
                blackScore.set(blackScore.get() - disksFlipped);
            }
            move.set(move.get() + 1);
    ...
    public String getBoardInConsoleString() {
        ...
        output.append(("""
                        ...
                        Score:  %-2d      %-2d        Move:  %-2d
                            ...
                            a   b   c   d   e   f   g   h""")
                        .formatted(getActivePlayer() == 0 ? "B" : "W", blackScore.get(), blackScore.get(), move.get()));
    ...
    public String getWinnerString() {
        if (blackScore.get() > whiteScore.get()) {
            return "Black won.";
        } else if (whiteScore.get() > blackScore.get()) {
            return "White won.";
        } else {
            return "Draw.";
        }
    ...
    public int getBlackScore() { return blackScore.get(); }

    public int getWhiteScore() { return whiteScore.get(); }

    public int getMove() { return move.get(); }

    public ReadOnlyIntegerProperty blackScoreProperty() {
        return blackScore.getReadOnlyProperty();
    }

    public ReadOnlyIntegerProperty whiteScoreProperty() {
        return whiteScore.getReadOnlyProperty();
    }

    public ReadOnlyIntegerProperty moveProperty() {
